...
export default function AttendanceScoresPage() {
  ...
  const handleCourseChange = async (courseId: string) => {
    ...
    setLoading(true);
    try {
      const response = await attendanceApi.getCourseAttendanceScores(courseId);
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Attendance Scores");

    worksheet.columns = [
      { header: "Student ID", key: "studentId", width: 15 },
      { header: "Name", key: "name", width: 25 },
      { header: "Email", key: "email", width: 30 },
      { header: "Sessions Attended", key: "sessions", width: 20 },
    ...
    filteredStudents.forEach((student) => {
      worksheet.addRow({
        studentId: student.userIdentifier,
        name: student.fullName,
        email: student.email,
        sessions: `${student.attendedSessions} / ${attendanceScores.totalSessions}`,
        rate: `${Math.round((student.attendedSessions / attendanceScores.totalSessions) * 100)}%`,
  ...
  const exportToCSV = async () => {
    ...
    worksheet.columns = [
      ...
      { header: "Sessions Attended", key: "sessions" },
      { header: "Attendance Rate", key: "rate" },
      { header: "Average Score", key: "avgScore" },
  ...
  const exportToPDF = () => {
    ...
    if (course) {
      doc.text(`Course: ${course.course_name} (${course.course_code})`, 14, 22);
      doc.text(`Total Sessions: ${attendanceScores.totalSessions}`, 14, 29);
    }
        ...
        {selectedCourseId && (
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div className="text-xl font-bold">Attendance Scores</div>
            ...
            <CardContent>
              ...
              {error && !loading && (
                <Alert variant="destructive" className="mb-4">
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle>Error Loading Attendance Scores</AlertTitle>
                      ...
                      {filteredStudents.length === 0 ? (
                        ...
                        filteredStudents.map((student) => (
                          <TableRow key={student.studentId}>
                            <TableCell>{student.userIdentifier}</TableCell>
                            <TableCell>
